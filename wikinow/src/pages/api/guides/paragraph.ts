...
export default async function handler(
  ...
  console.log(`[X] Checking paragraph for Guide: ${guideID}, Part: ${partID}, Step: ${stepID}`);
...
const checkParagraphStep = async (guide: string, step: string) => {
  ...
  if (substep.paragraph === null || substep.paragraph === "") {
    const cohereParagraph = (await generateParagraph(guide, step, substep.text)).body.generations[0]?.text.replaceAll("\n", " ").replaceAll("---", "");
