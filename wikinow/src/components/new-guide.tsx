...
const empty = () => toast.error('Title cannot be empty!', { position: 'bottom-right' });
...
export const NewGuide = () => {
    ...
    const handleCreateGuide = async () => {
        ...
        setIsCreating(true);
        toast.promise(createGuide.mutateAsync({ title: title }), {
    ...
    return <div className="container flex flex-col items-center justify-center gap-12 px-4 py-16 ">
        <div className="flex flex-row items-center justify-around text-xl sm:text-2xl">
            <span className="text-black pr-2">How to</span>
            <input type="text" value={title} onChange={(event) => {
                if (isEmpty) setIsEmpty(false);
                setTitle(event.target.value)
            }} className="bg-transparent border-black border-b-2 outline-none"></input>
        ...
        <Transition
            ...
            leaveTo="scale-0"
        >
            <span className="text-xl text-error-400">The {'"'}How To{'"'} question cannot be empty!</span>
        </Transition>
        ...
        <Transition
            ...
            leaveTo="opacity-0"
        >
            <div className="animate-spin rounded-full h-16 w-16 border-b-4 border-port_gore-500"></div>
        </Transition>

        <Transition
            show={!isCreating}
        >
            <PrimaryButton child={<span>Go!</span>} onClick={async () => await handleCreateGuide()} />
        </Transition>
    </div>
}