...
export const generateSteps = async (text: string) => {
  ...
  const stepsPrompt = await fs.readFile(
    path.join(staticDirectory, "steps-prompt.txt"),
    "utf8"
  );
...
export const generateParagraph = async (
  ...
  const staticDirectory = path.join(process.cwd(), "static");
  const paragraphsPrompt = await fs.readFile(
    path.join(staticDirectory, "paragraphs-prompt.txt"),
    "utf8"
  );
