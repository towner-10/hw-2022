...
export const guidesRouter = createTRPCRouter({
      ...
      for (const step of steps) {
        if (step.paragraph === null || step.paragraph === "") {
          const cohereParagraph = (
            await generateParagraph(guide.title, step.text, step.id)
  ...
  generateParagraph: publicProcedure
    .input(
      z.object({ question: z.string(), part: z.string(), step: z.string() })
    )
    .mutation(async ({ ctx, input }) => {
      ...
      const cohereParagraph = (
        await generateParagraph(input.question, input.part, input.step)
      ).body.generations[0]?.text
        .replaceAll("\n", " ")
        .replaceAll("---", "");
